...
/**
 * Entity đại diện cho bảng analytics.click_agg_minute — dữ liệu click gộp theo
 * phút.
 *
 * Cơ chế hoạt động:
 * - ClickIngestionService ghi raw event vào click_events.
 * - Scheduled job (mỗi phút) đọc click_events → GROUP BY link_id,
 * trunc(occurred_at, minute)
 * → UPSERT vào bảng này.
 * - Lưu tổng click, unique visitors, bot clicks + breakdowns (country, device,
 * referrer).
 * - Bảng này cũng dùng PARTITION BY RANGE — giữ 7 ngày gần nhất, data cũ hơn
 * roll-up vào daily.
 *
 * Tại sao gộp theo phút?
 * - Query real-time dashboard nhanh hơn đọc raw events.
 * - Giảm I/O khi vẽ biểu đồ timeseries (1440 row/ngày/link thay vì hàng triệu
 * events).
 */
@Entity
